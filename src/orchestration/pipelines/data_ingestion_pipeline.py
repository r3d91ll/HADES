"""
Complete Data Ingestion Pipeline for HADES-PathRAG
...
class DataIngestionPipeline:
    ...
    def _run_document_processing(self, input_files: List[str]) -> List[DocumentSchema]:
        ...
        try:
            ...
            self.stats["stage_times"]["document_processing"] = stage_time
        ...
        try:
            ...
            for chunk_schema in chunk_schemas:
                chunk_dict = {
                    ...
                    'source_document': getattr(chunk_schema, 'source_document', ''),
            ...
            stage_time = time.time() - stage_start
            self.stats["stage_times"]["chunking"] = stage_time
    ...
    def _run_embedding(self, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            stage_time = time.time() - stage_start
            self.stats["stage_times"]["embedding"] = stage_time
    ...
    def _run_isne_enhancement(self, embedded_chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for doc in isne_enhanced_documents:
                for chunk in doc.chunks:
                    chunk_dict = {
                        ...
                        'source_document': doc.file_name,
                        'embedding': chunk.embedding,
                        'isne_embedding': getattr(chunk, 'isne_embedding', None),
                        'metadata': chunk.metadata if hasattr(chunk, 'metadata') else {},
                        'embedding_metadata': {
                            'model': 'modernbert',
                            'dimension': len(chunk.embedding) if chunk.embedding else 0,
                        ...
                        'graph_relationships': getattr(chunk, 'graph_relationships', {}),
                        'relationships': {
                            'similar_chunks': [],
                            'sequential_chunks': [],
            ...
            stage_time = time.time() - stage_start
            self.stats["stage_times"]["isne"] = stage_time
    ...
    def _run_storage(self, final_chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for source_doc, chunk_list in doc_chunks_for_storage.items():
                ...
                for chunk in chunk_list:
                    ...
                    if 'isne_embedding' in chunk:
                        chunk_schema.isne_embedding = chunk['isne_embedding']
            ...
            stage_time = time.time() - stage_start
            self.stats["stage_times"]["storage"] = stage_time
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run HADES-PathRAG data ingestion pipeline")
