#!/usr/bin/env python3
"""
Comprehensive ISNE Training Script V2
...
This version copies files instead of symlinking for better compatibility.
"""
...
def prepare_input_directory_batch(sources: Dict[str, List[str]], batch_size: int = 100) -> Path:
    """Prepare input directory with a batch of files (copy instead of symlink)."""
    ...
    # Process PDFs first (all of them since there are only ~382)
    pdf_count = 0
    for pdf_path in sources["pdfs"]:
        pdf_file = Path(pdf_path)
        dest_path = input_dir / pdf_file.name
        try:
            shutil.copy2(pdf_file, dest_path)
            pdf_count += 1
        except Exception as e:
            logger.warning(f"Failed to copy PDF {pdf_file}: {e}")
    
    logger.info(f"Copied {pdf_count} PDF files to input directory")
    
    # Process a batch of Python files
    py_count = 0
    for py_path in sources["python_files"][:batch_size]:
        py_file = Path(py_path)
        # Create a unique name to avoid conflicts
        unique_name = f"{py_file.parent.name}_{py_file.name}"
        dest_path = input_dir / unique_name
        try:
            shutil.copy2(py_file, dest_path)
            py_count += 1
        except Exception as e:
            logger.warning(f"Failed to copy Python file {py_file}: {e}")
    
    logger.info(f"Copied {py_count} Python files to input directory")
    logger.info(f"Total files in input directory: {pdf_count + py_count}")
...
def run_comprehensive_pipeline(sources: Dict[str, List[str]]) -> None:
    ...
    # You can increase this to process all files once it's working
    INITIAL_BATCH_SIZE = 100
    
    # Prepare input directory with batch of files
    input_dir = prepare_input_directory_batch(sources, batch_size=INITIAL_BATCH_SIZE)
    
    # Count total files
    total_files = len(sources["pdfs"]) + len(sources["python_files"])
    logger.info(f"Total available files: {total_files}")
    logger.info(f"Processing initial batch of {INITIAL_BATCH_SIZE} Python files + all PDFs")
    ...
    pipeline_config = {
        "docproc": {
            ...
            "config": {
                "pdf_processor": "core",  # Using core instead of marker for reliability
        ...
        "embedding": {
            "implementation": "cpu",  # Start with CPU for reliability
            "config": {
    ...
    try:
        ...
        # Use poetry to run the script with all dependencies
        cmd = [
            "poetry", "run", "python",
    ...
    summary = {
        ...
        "files_available": total_files,
        "files_processed_batch": INITIAL_BATCH_SIZE + len(sources["pdfs"]),
