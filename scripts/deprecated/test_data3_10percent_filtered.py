#!/usr/bin/env python3
"""
Test-Data3 10% Sample Test with Empty Document Filtering

Enhanced version that filters out empty documents to avoid chunking errors.
Uses a 10% representative sample from test-data3 dataset.
"""
...
from src.isne.bootstrap.stages.document_processing import DocumentProcessingStage
...
class FilteredISNEBootstrapPipeline(ISNEBootstrapPipeline):
    """Enhanced pipeline that filters empty documents."""
    
    def _run_document_processing(self, stage, input_files, config):
        """Run document processing stage with empty document filtering."""
        # Run the original processing
        result = super()._run_document_processing(stage, input_files, config)
        ...
        if result.success and result.documents:
            ...
            for doc in result.documents:
                if hasattr(doc, 'content') and doc.content and doc.content.strip():
                    ...
                    empty_count += 1
                    self.logger.debug(f"Filtered empty document: {getattr(doc, 'id', 'unknown')}")
            
            if empty_count > 0:
                self.logger.info(f"Filtered out {empty_count} empty documents from {original_count} total")
            
            # Update the result with filtered documents
            result.documents = filtered_documents
            result.stats['empty_documents_filtered'] = empty_count
            result.stats['documents_after_filtering'] = len(filtered_documents)
            
            # Check if we still have documents
            if not filtered_documents:
                result.success = False
                result.error_message = "All documents were empty after filtering"
...
def run_10_percent_filtered_test():
    """Run test with 10% sample from test-data3 with empty document filtering."""
    
    print("🎯 TEST-DATA3 10% SAMPLE TEST (WITH FILTERING)")
    print("="*70)
    print("🚀 Testing with representative 10% sample + empty doc filtering:")
    ...
    print("   ✅ Enhancement: Empty document filtering")
    print()
    print("🎯 Goal: Validate approach with clean data")
    print("   Sample: 10% of each file type for balanced representation")
    print("   Target: >60% inductive performance to validate approach")
    print("="*70)
    
    # Set up paths
    output_dir = Path("output/test_data3_10percent_filtered")
    model_name = "test_data3_10percent_filtered_model"
    ...
    config.pipeline_name = "test_data3_10percent_filtered"
    ...
    # Validation settings (even though we're filtering)
    ...
    # Use our filtered pipeline
    pipeline = FilteredISNEBootstrapPipeline(config, monitor)
    
    print(f"\n🚀 Starting 10% sample test with filtering...")
    print(f"⏱️  Started at: {time.strftime('%H:%M:%S')}")
    print(f"💡 Empty documents will be filtered automatically")
...
def analyze_sample_results(output_dir: str, num_input_files: int):
    ...
    if results_file.exists():
        ...
        # Document filtering stats
        if 'document_processing_stats' in eval_results:
            dp_stats = eval_results['document_processing_stats']
            if 'empty_documents_filtered' in dp_stats:
                empty_filtered = dp_stats['empty_documents_filtered']
                docs_after = dp_stats.get('documents_after_filtering', 0)
                print(f"\n📋 Document Filtering Stats:")
                print(f"   Empty documents filtered: {empty_filtered}")
                print(f"   Documents after filtering: {docs_after}")
...
if __name__ == "__main__":
    print("🔬 TEST-DATA3 10% SAMPLE TEST (FILTERED)")
    print("Efficient validation with empty document filtering")
    print("Testing all quick wins before full-scale training")
    print()
    
    success = run_10_percent_filtered_test()
    
    print(f"\n{'='*70}")
    print(f"10% FILTERED TEST: {'✅ SUCCESS' if success else '❌ FAILED'}")
    print(f"{'='*70}")
