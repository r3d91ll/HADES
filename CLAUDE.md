...
# With custom config
python scripts/bootstrap_jinav4.py /path/to/documents --config src/config/isne/isne_bootstrap_config.yaml
...
All components use type definitions from `src/types/`. When adding new features:

1. Define types in the appropriate module under `src/types/`
2. Implement the component using these types
...
- `src/config/` - Mirrors src/ structure exactly
- `src/config/jina_v4/config.yaml` - Jina v4 settings
- `src/config/isne/` - ISNE configurations
- `src/config/storage/` - Storage backend settings
- `src/config/pathrag/` - PathRAG parameters
...
The system uses Jina v4's native multimodal capabilities for document processing:

- **Multimodal Parsing**: Jina v4 will handle text, images, and structured documents in a unified way
- **Fallback Parsers**: Currently using PyPDF2 and python-docx as temporary fallbacks
- **Future Implementation**: The `_parse_with_jina_v4()` method will leverage Jina v4's built-in OCR and layout understanding
...
2. Add appropriate parser or extend Jina v4 multimodal parsing
